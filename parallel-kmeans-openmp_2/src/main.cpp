...
void print_help(const char *program) {
    std::cout
        ...
        << "  --mode <serial|parallel|optimized> 运行模式，默认 parallel\n"
...
ProgramOptions parse_args(int argc, char *argv[]) {
    ...
    if (options.mode != "serial" && options.mode != "parallel" && options.mode != "optimized") {
        throw std::invalid_argument("mode 只能是 serial、parallel 或 optimized");
    }
...
int main(int argc, char *argv[]) {
    try {
        ...
        } else if (options.mode == "optimized") {
            result = kmeans_optimized(data, config);
        } else {
        ...
        std::cout << options.mode << ','
                  ...
                  << std::setprecision(6) << result.pruning_rate << '\n';
