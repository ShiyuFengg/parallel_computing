...
echo "n,dim,k,mode,threads,time,iterations,inertia,speedup,efficiency" > "$RESULT_FILE"
...
append_result() {
    ...
    IFS=',' read -r mode_out n_out dim_out k_out _max_iter_out threads_out iterations_out time_out inertia_out <<< "$raw"
    echo "${n_out},${dim_out},${k_out},${mode_out},${threads_out},${time_out},${iterations_out},${inertia_out},${speedup},${efficiency}" >> "$RESULT_FILE"
}
...
for n in "${NS[@]}"; do
    ...
    serial_raw="$("$BIN" --mode serial \
                          ...
                          --threads 1 \
                          --seed "$SEED")"
    IFS=',' read -r _mode _n _dim _k _max_iter _threads _iterations serial_time _inertia <<< "$serial_raw"
    ...
    for threads in "${THREADS[@]}"; do
        ...
        parallel_raw="$("$BIN" --mode parallel \
                                ...
                                --threads "$threads" \
                                --seed "$SEED")"
        IFS=',' read -r _mode _n _dim _k _max_iter _threads _iterations parallel_time _inertia <<< "$parallel_raw"
